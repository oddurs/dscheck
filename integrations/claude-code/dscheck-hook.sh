#!/bin/sh
# dscheck guardrail for Claude Code.
#
...
# project's .claude/settings.json (adjust the dscheck path if not on PATH).
...
findings=$(dscheck check "$file" --format agent 2>/dev/null)
[ -z "$findings" ] && exit 0
