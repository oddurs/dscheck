...
const dir = mkdtempSync(join(tmpdir(), 'dscheck-sl-'));
...
describe('stylelint-dscheck', () => {
  it('reports raw colors with the nearest token', async () => {
    ...
    expect(warnings[0]?.rule).toBe('dscheck/no-raw-color');
  ...
  it('reports raw lengths and unknown tokens independently', async () => {
    ...
    expect(rules).toEqual(['dscheck/no-raw-length', 'dscheck/no-unknown-token']);
    expect(warnings.find((w) => w.rule === 'dscheck/no-unknown-token')?.text).toContain(
...
describe('autofix', () => {
  it('fixes exact matches only', async () => {
    ...
    await stylelint.lint({
      ...
      config: {
        plugins: [join(import.meta.dirname, '..', 'dist', 'index.js')],
        rules: { 'dscheck/no-raw-color': true, 'dscheck/no-raw-length': true },
      },
    });
