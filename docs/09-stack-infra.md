# dscheck — Tech stack, integrations, infra

*Status: draft v1, 2026-08-23. Guiding principle from research ([06](06-competitors-lint.md)): every SaaS in this space died (Specify, Backlight, Omlet-hosted). dscheck is OSS, CLI-first, **zero servers** — infra is registries, static pages, and CI.*
...
| Repo lint | biome + dscheck on itself (dogfood from day 1) | fast; the dogfood matters |
...
**CI / code review**
- **GitHub Actions** — `dscheck/action@v1`: install, run, SARIF upload (`github/codeql-action/upload-sarif`), optional sticky PR trend comment. Code Scanning fingerprints give "new findings only" on PRs with no baseline file.
...
- MCP (P3): `dscheck mcp` — `lint_file`, `query_tokens`, `nearest_token` — so any MCP-speaking agent can query the allowed set *before* writing.
...
- No hosted dashboard/API/DB — the trend report is a CLI artifact + PR comment; if demand appears, a static HTML report (`dscheck report --html`) ships before any server does.
